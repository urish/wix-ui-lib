module.exports = function (grunt) {
	...
	grunt.initConfig({
		...
		watch: {
		 ...
		 markdown : {
		 	docs : {
		 		...
		 		options : {
		 			template : 'docs/markdownTemplate.tpl',
		 			preCompile : function (src, context) {
						context.parts = getDocMdContent('', src);
						return replaceMdParts(src, context.parts);
					},
		...
		mdDocs:{
			options: {
				files:['components/**/*.md'],
				dest: 'build/docs/docs.html',
				inject: 'build/docs/components.html' 
			}
		},
		clean : ['build']
	...
	grunt.registerTask('mdDocs', 'build docs', function () {
		...
		grunt.log.ok('writing: '+ options.dest);
		grunt.file.write(options.dest,all.join('\n\n\n\n'));
