(function ($, window, document, undefined) {
	...
	Plugin.prototype.showFirst = function () {
		...
		//this.$el.find('.' + opt.triggerClass + ':first').addClass(opt.activeClass).next('.' + opt.contentClass).show();
        this.$el.find('.' + opt.triggerClass + ':first').find('.acc_content').css('display','block')
	};
	...
	Plugin.prototype.bindEvents = function () {
		...
		this.$el.on('click', '.' + opt.triggerClass, function (e) {
			...
			if ($this.find('.' + opt.contentClass).is(':hidden')) {
				...
				$this.toggleClass(opt.activeClass).find('.'+opt.contentClass).slideDown(opt.animationTime, opt.ease);
			}
			return false;
		});
	};
